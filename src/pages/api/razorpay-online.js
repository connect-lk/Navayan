...
export default async function handler(req, res) {
    ...
    try {
        const { amount, planName, bookingDetails } = req.body;
        ...
        // Generate unique receipt ID for online payment
        const receiptId = `receipt_online_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
        ...
        const options = {
            ...
            notes: {
                payment_type: "online",
                plan_name: planName || "Booking Payment",
                ...(bookingDetails && { booking_details: JSON.stringify(bookingDetails) }),
            },
        };

        console.log("Creating Razorpay online payment order with options:", {
        ...
        console.error("Razorpay Online Payment API Error:", error);
